...
const start = async () => {
  try {
    ...
    const { runEmergencyMigrations, runGoogleCalendarMigrations, runSubscriptionMigrations, runStorefrontMigrations, runStorefrontExpansionMigrations } = await import('./db/migrations.js');
    ...
    await runStorefrontExpansionMigrations();
